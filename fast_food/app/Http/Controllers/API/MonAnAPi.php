...
class MonAnApi extends Controller
{
    ...
    public function index()
    {
        ...
        foreach($monAn as $item){
            ...
            $item->hinhAnhs=HinhAnh::where('mon_an_id',$item->id)->get();
    ...
    public function ShowDanhGia(MonAn $monAn){
       $monAn->danhGia= DanhGia::where('mon_an_id',$monAn->id)->get();
       $monAn->loaiMonAn =LoaiMonAn::find($monAn->loai_mon_an_id);
       foreach($monAn->DanhGia as $item){
    ...
    public function ShowUserDanhGia(User $user){
        $user->danhGia= DanhGia::where('user_id',$user->id)->get();
        foreach($user->DanhGia as $item){
    ...
    public function Star(MonAn $monAn){
      $sum= $monAn->danhGia =DanhGia::where('mon_an_id',$monAn->id)->get();
      $tong= $sum->sum('danh_gia_sao');
      $avg=$tong/count($sum);
    //   $avg= $sum/count($sum);
        return [
    ...
    {    
        ...
        $monAn->danhGia=round($tong/$count,1) ;
        return $monAn;
    }
